...
class PatchAutoEncoder(torch.nn.Module, PatchAutoEncoderBase):
    ...
    class PatchEncoder(torch.nn.Module):
        ...
        def __init__(self, patch_size: int, latent_dim: int, bottleneck: int):
            super().__init__()
            # Patchify input with convolution
            self.patch_conv = torch.nn.Conv2d(3, latent_dim, patch_size, patch_size, bias=False)
            # Add non-linearities and interactions between patches
            self.conv1 = torch.nn.Conv2d(latent_dim, latent_dim, 3, 1, 1)
            self.gelu1 = torch.nn.GELU()
            self.conv2 = torch.nn.Conv2d(latent_dim, bottleneck, 1, 1, 0)
    ...
    class PatchDecoder(torch.nn.Module):
        ...
        def forward(self, x: torch.Tensor) -> torch.Tensor:
            # x: (B, h, w, bottleneck) -> (B, bottleneck, h, w)
            x = hwc_to_chw(x)
            x = self.gelu1(self.conv1(x))
            x = self.conv2(x)
            # Unpatchify
